...
# Setting ffmpeg codec switch to "x265" with CodecSwitch
CodecSwitch="x265"
# Setting ffmpeg preset
PRESET="slow"
# Resetting variables
LogOutput="##########################################\n"
FullHelpTip=""
...
#ffmpeg -n -hide_banner -i P1010301.mp4 -c:v libx264 -preset veryslow -crf 21 -c:a copy x264crf21-P1010301.mp4

#=======================================================================
# Help options
#=======================================================================
HelpTip="Help: for more parameters use '/bin/bash ${InstanceName} <-h|--help>'\n"
UsageTip="Usage: '/bin/bash ${InstanceName} <source-path|.> <destination-path|.> <x264|x265> <medium|slow|slower|verslow> <--copy|--move> <--timerON|--timerOFF> <--crawlON|--crawlOFF> <--DateModel|--ModelDate|--NoModel>\n  Mandatory parameters: source-path, destination-path\n"
...
if [[ "${#}" -gt 2 ]] ; then
  ...
  for Argument in "${@:3}" ; do
    case "${Argument}" in 
      ...
      --ExT)
        # Setting extension case switch to "ExT" with CodecSwitch, i.e. changing file extension case to UnChanged
        CodecSwitch="ExT"
        ;;
      # Checking if the argument is an extension case switch and validating it
      --EXT)
        # Setting extension case switch to "EXT" with CodecSwitch, i.e. changing file extension case to UPPERCASE
        CodecSwitch="EXT"
        ;;
      # Checking if the argument is an extension case switch and validating it
      --ext)
        # Setting extension case switch to "ext" with CodecSwitch, i.e. changing file extension case to lowercase
        CodecSwitch="ext"
        ;;
      ...
      --YMD)
        # Setting Target Directory structure with OriginalVideosSubfolder to YMD = YEAR/MONTH/DAY
        OriginalVideosSubfolder="YMD"
        ;;
      # Checking if the argument is a directory structure switch and validating it
      --YM)
        # Setting Target Directory structure with OriginalVideosSubfolder to YM = YEAR/MONTH
        OriginalVideosSubfolder="YM"
        ;;
      # Checking if the argument is a directory structure switch and validating it
      --Y)
        # Setting Target Directory structure with OriginalVideosSubfolder to Y = YEAR
        OriginalVideosSubfolder="Y"
        ;;
      # Checking if the argument is a directory structure switch and validating it
      --NOSORT)
        # Setting Target Directory structure with OriginalVideosSubfolder to NOSORT = All file in the root of Destination
        OriginalVideosSubfolder="NOSORT"
        ;;
...
case "${OriginalVideosSubfolder}" in 
  YMD)
...
if [[ ${PrerequisitesOK} -eq 1 ]] && [[ ${FilesFetched} -eq 1 ]] ; then
  ...
  for WIPSortedFileAbsolutePath in ${WIPSortedFileAbsolutePaths[@]}; do
    ...
    if [ -e "${WIPSortedFileAbsolutePath}" ] && [ "${#WIPSortedFileAbsolutePath}" != 0 ] ; then
      ...
      # Change extension lowercase if CodecSwitch is set to "ext"
      if [[ "${CodecSwitch}" == 'ext' ]] ; then
        NormalisedFileExtension="$(echo "${WIPFileExtension}" | awk '{print tolower($0)}')"
      else
      # Change extension uppercase if CodecSwitch is set to "EXT"
        if [[ "${CodecSwitch}" == 'EXT' ]] ; then
      ...
      # Setting Target Directory structure with OriginalVideosSubfolder
      case "${OriginalVideosSubfolder}" in 
        # YMD = YEAR/MONTH/DAY
        YMD)
